import numpy as np
from "../CS517_Semester_project/src/Least_Squares_Approximation.py"
...
def save_to_file(core: np.ndarray, slope: np.ndarray, times: np.ndarray, core_idx: int):
    ...
    with open(output_file, "w") as newFile:
        ...
        for i in range(len(slope)):  # Loop over the slopes
            ...
            newFile.write(
                f"{t0:.2f} <= x <= {t1:.2f}; y = {core[i]:.4f} + {slope_str:>{max_slope_length}} x\n"
            )

        # Write least squares line at the end
        newFile.write(
            f"Overall (Least Squares): {times[0]:.2f} <= x <= {times[-1]:.2f}; "
            f"y = {core_ls_intercept:.4f} + {core_ls_slope:.4f} x\n"
        )
