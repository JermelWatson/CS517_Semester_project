...
if __name__ == "__main__":
    ...
    if len(times) == len(core_0_slope):
        for i, (t0, t1) in enumerate(zip(core_0_slope[:-1], core_0_slope[1:])):
            print(f"{i+1}: {times[i]} <= x <= {times[i + 1]}; y= \t {temps[i]} + \t {core_0_slope[i]} x ; interpolation")
            pass