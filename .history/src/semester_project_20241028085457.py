...
def save_to_file(core: list[float], slope: list[float], times: list[float]):
    ...
    max_slope_length = max(len(f"{slope:.4f}") for slope in slope)  # Including the negative sign
    
    for i in range(min(len(core))):# Loop over the slopes
        ...
        newFile.write(f"{t0:>3} <= x <= \t{t1:>10}; y = \t{core[i]:>10.4f} + \t\t\t{slope_str:>{max_slope_length}}{' x ; interpolation' if slope >= 0 else ' x ; interpolation'}\n")
        #Send data to file
        newFile.close()
