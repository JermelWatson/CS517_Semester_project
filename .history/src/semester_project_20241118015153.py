...
def save_to_file(core: list[float], slope: list[float], times: list[float], core_idx: int):
    ...
    newFile.write(f"{times[0]:>3} <= x <= \t{times[-1]:>10}; y = \t{core_0_ls_intercept:.4f} + \t\t\t{core_0_ls_slope:>{max_slope_length}}{'x ; least-squares' if core_0_ls_slope >=0 else 'x ; least-squares')}\n
    newFile.close()
